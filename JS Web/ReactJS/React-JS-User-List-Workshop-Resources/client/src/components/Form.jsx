...
export default function Form(props) {
    ...
    if (props.showForm !== 'Add') {
        const userId = props.showForm;
        useEffect(() => {

            async function getData() {
                try {
                    const response = await requests.getOneUser(userId);
    ...
    async function submitHandler(e) {
        ...
        try {
            ...
            if (props.showForm !== 'Add') {
                const userId = props.showForm;
                await requests.editUser(userId, formData);

                const users = await requests.getAllUsers();

                return props.setUsers(users);
            }
            await requests.createUser(formData);
            ...
            <div className="modal">
                <div className="user-container">
                    <header className="headers">
                        <h2>{props.showForm === 'Add' ? 'Add User' : 'Edit User'}</h2>
                        <button className="btn close">
                            <svg onClick={() => props.setShowForm(false)} aria-hidden="true" focusable="false" data-prefix="fas" data-icon="xmark"
                    ...
                    <form onSubmit={submitHandler}>
                        ...
                        <div id="form-actions">
                            ...
                            <button onClick={() => props.setShowForm(false)} id="action-cancel" className="btn" type="button">Cancel</button>
                        </div>
                    </form>
                </div>
            </div>
