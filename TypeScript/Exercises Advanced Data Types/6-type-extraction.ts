...
function createCombinedFunction(names: Name, location: Location) {
    ...
    return function combinedFunction(person: Name & Location) {
        console.log(`Hello, ${person.getPersonInfo()} from ${person.getAddressInfo()}`);
    }
}

let combinedFunction = createCombinedFunction(names, location); let combinedPerson = Object.assign({}, names, location); combinedFunction(combinedPerson);