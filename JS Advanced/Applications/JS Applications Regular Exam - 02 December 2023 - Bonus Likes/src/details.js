...
export async function detailsView(ctx) {
  ...
  //get likes
  const likesUrl = `http://localhost:3030/data/useful?where=characterId%3D%22${id}%22&distinct=_ownerId&count`;
  ...
  if (isLogged) {
    const userData = JSON.parse(localStorage.getItem('userData'));
    const checkLikeUrl = `http://localhost:3030/data/useful?where=characterId%3D%22${id}%22%20and%20_ownerId%3D%22${currentOwnerId}%22&count`;
