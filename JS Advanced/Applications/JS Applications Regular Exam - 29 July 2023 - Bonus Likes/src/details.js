...
const template = (arr, isOwner, isLogged, likes, likeHandlerTag, isLiked) => html`
      <section id="details">
        <div id="details-wrapper">
          ...
          <p id="details-category">${arr.category}</p>
          <div id="info-wrapper">
...
export async function detailsView(ctx) {
  ...
  //get likes
  const likesUrl = `http://localhost:3030/data/likes?where=factId%3D%22${id}%22&distinct=_ownerId&count`;
  ...
  if (isLogged) {
    const userData = JSON.parse(localStorage.getItem('userData'));
    const checkLikeUrl = `http://localhost:3030/data/likes?where=factId%3D%22${id}%22%20and%20_ownerId%3D%22${currentOwnerId}%22&count`;
...
async function likeHandler() {
  ...
  const responseLikes = await fetch(likeUrl, {
    method: 'POST',
    body: JSON.stringify({
      factId: id //  the name of the property will probably be different
    }),
    headers: {
